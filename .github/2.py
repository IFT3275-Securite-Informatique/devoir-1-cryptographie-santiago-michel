...
# Remplace par ta liste complète de 256 symboles

# Calcul des fréquences uniquement pour les symboles dans la liste prédéfinie
# Compte les caractères seuls et les bigrammes dans le texte
frequences = Counter(char for char in texte_1000_mots if char in symboles_256)
frequences.update(pair for pair in cut_string_into_pairs(texte_1000_mots) if pair in symboles_256)

# Affiche les résultats de fréquence pour les symboles définis
print("Fréquence des symboles prédéfinis dans le texte de 1000 mots:")
for symbole, freq in frequences.most_common():
    print(f"Symbole: {symbole}, Fréquence: {freq}")


#########
def count_unique_8bit_sequences(code):
    # Diviser le code en séquences de 8 bits
    sequences = [code[i:i+8] for i in range(0, len(code), 8)]
    
    # Utiliser un ensemble pour compter les séquences uniques
    unique_sequences = set(sequences)
    
    # Retourner le nombre de séquences uniques
    return len(unique_sequences), unique_sequences

##########

def map_most_frequent_sequences(code):
    # Diviser le code en séquences de 8 bits
    sequences = [code[i:i+8] for i in range(0, len(code), 8)]
    ...
    # Trier les séquences par fréquence décroissante
    most_common_sequences = sequence_counts.most_common()
    
    # Vérifier que nous avons bien des tuples (séquence, fréquence)
    for item in most_common_sequences:
        if not isinstance(item, tuple) or len(item) != 2:
            print("Erreur : la liste des séquences les plus fréquentes n'est pas bien formée.")
            return code, None, None
    
    # Identifier les séquences les plus fréquentes
    if len(most_common_sequences) >= 2:
        espace_sequence = most_common_sequences[0][0]  # La plus fréquente pour l'espace
        e_sequence = most_common_sequences[1][0]       # La deuxième pour 'e'
        print(f"Séquence la plus fréquente (remplacée par espace) : {espace_sequence}")
        print(f"Deuxième séquence la plus fréquente (remplacée par 'e') : {e_sequence}")
    else:
        print("Le code ne contient pas assez de séquences pour effectuer le mapping.")
        return code, None, None
    
    return code, espace_sequence, e_sequence, most_common_sequences

def remplacer_8bits_par_texte(code, espace_sequence, e_sequence):
    # Remplacer les séquences dans le code
    code_decoded = code
    if espace_sequence:
...
nombre_sequences, sequences_uniques = count_unique_8bit_sequences(C)
print(f"Nombre de chaînes de 8 bits distinctes : {nombre_sequences}")
print("Séquences de 8 bits uniques :", sequences_uniques)


# Appeler la fonction pour obtenir les séquences à remplacer
code, espace_sequence, e_sequence, most_common_sequences = map_most_frequent_sequences(C)

# Afficher les séquences les plus fréquentes pour vérification
if espace_sequence and e_sequence:
    print("\nSéquences les plus fréquentes et leurs occurrences :")
    for seq, count in most_common_sequences[:10]:  # Afficher les 10 premières pour plus de détails
        print(f"Séquence: {seq}, Fréquence: {count}")

    # Imprimer le code modifié
    code_decoded = remplacer_8bits_par_texte(C, espace_sequence, e_sequence)
    print("\nCode après remplacement :")
    print(code_decoded)
